...
//beforeSend
            if (id) {
                client.put(`/categoria/update/${id}`, { categoria }).then(r => {
                    store.showSuccess({ message: "Categoria Actualizado", redirect: "/dashboard/categoria/list", navigate });
                }).catch(e => {
                ...
                client.post(`/categoria/create`, { categoria }).then(r => {
                    store.showSuccess({ message: "Categoria Creado", redirect: "/dashboard/categoria/list", navigate });
                }).catch(e => {
...
//convertRead
    ...
    return (
            ...
            <div className="px-2 py-1 sm:py-2 xl:py-4">
                <div className="bg-gray-50 px-1  py-0.5 sm:py-0.5 2xl:py-1  sm:flex flex flex-wrap-reverse sm:items-center w-full">
                    <div className="sm:flex-auto">
                        <h1 className="ttext-balance 2xl:text-xl sm:text-lg text-sm  mt-2 sm:mt-0 font-semibold text-gray-900 uppercase">{id ? "EDITAR Categoria" : "REGISTRAR Categoria"}</h1>
                    </div>
        ...
        <div className="">
            <label htmlFor="nombre_categoria" className="block font-medium text-black 2xl:text-sm text-xs">
                Nombre Categoria
            </label>
            <div className="relative rounded-md shadow-sm">
                <div className="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3">
                    <Icon name="Bars2Icon" className="h-5 w-5 text-gray-400" aria-hidden="true" />
                </div>
                <input type="text" name="nombre_categoria" value={categoria?.nombre_categoria} onChange={e => set_categoria({ ...categoria, nombre_categoria: e.target.value })} required  className="pl-10 focus:ring-blue-300 focus:border-blue-300 block w-full shadow-sm 2xl:text-sm text-xs border-gray-300 rounded-md uppercase" />
            </div>
        </div>
...
{/* initJSXdescripcion */}
        ...
        <div className="">
            ...
            <textarea name="descripcion" value={categoria?.descripcion} onChange={e => set_categoria({ ...categoria, descripcion: e.target.value })}  rows="3" className="focus:ring-blue-300 focus:border-blue-300 block w-full shadow-sm 2xl:text-sm text-xs border-gray-300 rounded-md "/>
        </div>
