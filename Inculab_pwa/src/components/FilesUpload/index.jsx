...
const FilesUpload = forwardRef(({
  ...
  const renderThumbnail = (file, index, isInitial = false) => {
    ...
    if (file.type.startsWith('image/')) {
      ...
      return (
        <div key={index} className={`relative ${borderClass} rounded-md shadow-xl`}>
          <DocumentIcon className="w-28 h-28 text-gray-500 rounded-md icon-style" />
          {renderFileDetails(file, index, isInitial)}
        </div>
      );
    }
        ...
        {isInitial ?
          <>
            <a href={file.url} download target='_blank'
              ...
              <CloudArrowDownIcon className="h-10 w-10 text-white p-2 group-hover:text-green-500 icon-style" />
            </a>
            <button type="button" className="group absolute top-0 right-0 p-1 flex justify-center items-center bg-red-500 bg-opacity-80 rounded-full hover:bg-white"
              ...
              <TrashIcon className="h-5 w-5 text-white group-hover:text-red-500 icon-style" />
            </button>
          </>
          :
          <button type="button" className="group absolute top-0 right-0 p-1 flex justify-center items-center bg-red-500 bg-opacity-80 rounded-full hover:bg-white"
  ...
  return (
    <>
      ...
      <IonItem>
        <div className="p-4 border border-gray-300 rounded-lg max-h-80 shadow-md overflow-y-auto files-upload-container"
          ...
          {isDragging && (
            <div className="overlay">
              <div className="overlay-content flex">
                <CloudArrowUpIcon className="h-12 w-12 mr-4 icon-style" />
          ...
          {(localFiles.length > 0 || files.length > 0) ?
            <div className="flex flex-wrap gap-2 p-1">
              ...
              <div className="w-28 h-28 flex-none flex justify-center items-center rounded-md border-2 border-dashed border-green-500 cursor-pointer" onClick={addMoreFiles}>
                <PlusIcon className="h-8 w-8 text-green-500 icon-style" />
              </div>
            ...
            <div className='flex flex-col items-center justify-center h-full'>
              <DocumentIcon className='h-8 w-8 icon-style' />
              <p className="text-theme-adaptive">Arrastra o <strong className='text-blue-800 cursor-pointer' onClick={addMoreFiles}>Selecciona</strong> el archivo</p>
            </div>
          }
        </div>
      </IonItem>
